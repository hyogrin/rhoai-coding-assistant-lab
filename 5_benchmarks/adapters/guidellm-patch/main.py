...
def main() -> None:
    ...
    try:
        ...
        # Save to MLflow (creates run with metrics, params, artifacts)
        run_id = callbacks.mlflow.save(results, adapter.job_spec)
        if run_id:
            results.mlflow_run_id = run_id
            logger.info(f"MLflow run: {run_id}")

        # Log traces to MLflow Traces tab
        mlflow_direct_uri = os.getenv(
            "MLFLOW_DIRECT_URI", "https://mlflow.redhat-ods-applications.svc:8443"
        )
        enable_tracing = os.getenv("ENABLE_TRACING", "true").lower() == "true"
        if enable_tracing and run_id and results.evaluation_metadata:
            try:
                import mlflow
                ...
                logger.info(f"Logging traces to MLflow at {mlflow_direct_uri}")

                exp_id = mlflow_client.get_run(run_id).info.experiment_id

                # Log benchmark execution as a trace
                root_span = mlflow_client.start_trace(
                    name=f"guidellm_{adapter.job_spec.benchmark_id}",
                    experiment_id=exp_id,
                    inputs={
                        "model": adapter.job_spec.model.name,
                        "benchmark_id": adapter.job_spec.benchmark_id,
                        "target_url": adapter.job_spec.model.url,
                    },
                    attributes={
                        "profile": str(results.evaluation_metadata.get("profile", "")),
                        "duration_seconds": str(results.duration_seconds),
                        "num_requests": str(results.num_examples_evaluated),
                    },
                )
                trace_id = root_span.request_id

                # Include key metrics in trace output
                metrics_output = {}
                if results.results:
                    for r in results.results:
                        metrics_output[r.metric_name] = r.metric_value
                ...
                mlflow_client.end_trace(
                    ...
                    attributes={
                        "status": "success",
                        "overall_score": str(results.overall_score or 0),
                    },
                )
                logger.info("Trace logged to MLflow Traces tab")

            except Exception as e:
                logger.warning(f"Failed to log traces to MLflow: {e}")
