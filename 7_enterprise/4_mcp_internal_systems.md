...
Centralize non-secret MCP configuration in ConfigMaps managed by the platform team — the same GitOps workflow as Continue and Roo Code configs in Phase 4.
...
→ Register servers with MaaS (Phase 2) and add MCP endpoints to the Phase 4 Continue ConfigMap.
