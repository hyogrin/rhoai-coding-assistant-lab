...
In this lab, Dev Spaces becomes the **team onboarding layer**: every developer gets an identical workspace with AI coding extensions pre-configured to reach the MaaS gateway — no manual IDE setup required.
...
These flags are set via `VLLM_ADDITIONAL_ARGS` on the `InferenceService`:
...
5. Extensions call the MaaS gateway using OpenAI-compatible APIs.
