...
flowchart TB
    ...
    MODEL[MaaS / llm-d Gateway<br/>Qwen3-Coder]
...
Global prompts are distributed through **extension ConfigMaps** mounted into every DevWorkspace — the same pattern established in Phase 5.

### Continue — `systemMessage` in `config.yaml`

```yaml
# Fragment of continue-config ConfigMap (Phase 5 pattern)
data:
  config.yaml: |
    models:
...
Use the DevWorkspace controller annotations from Phase 5 so updates propagate without image rebuilds:
...
│   │   └── 30-security-sonar.md           # SonarQube rules (see Phase 8 doc 3)
│   ├── rules-architect/
...
> **Prompt length vs. prefix cache:** Shared global prompts increase prefix-cache hit rates on vLLM (Phase 7). Keep global prompts stable and identical across developers; put volatile or repo-specific content in per-project rules.
...
> **Cline caveat:** Cline reads `.clinerules` but global config still requires manual UI setup (Phase 5). Document the expected `.clinerules` location in team onboarding docs.
