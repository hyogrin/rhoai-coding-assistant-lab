...
Phases 0–6 deploy a single-GPU coding assistant with llm-d routing, MaaS gateway access, and team-scale developer onboarding. **Phase 7** addresses production-grade concerns that emerge when you move beyond a single cluster and single accelerator type:
...
The most advanced deployment pattern combines **NVIDIA GPUs** (low latency, FP8, prefix caching) with **Inferentia2** (overflow capacity, cost-effective batch inference) behind a single llm-d Endpoint Picker:
...
1. Client sends a chat completion request through the MaaS / llm-d gateway.
...
Scaling llm-d deployments follows a tiered approach:
...
| Component | Purpose |
|-----------|---------|
| Phases 0–4 completed | llm-d deployed with LLMInferenceService CRD |
| Phase 6 benchmarks | Baseline TTFT / throughput for capacity decisions |
...
| `2_multi_accelerator.md` | Heterogeneous llm-d routing (NVIDIA + Inferentia2) |
