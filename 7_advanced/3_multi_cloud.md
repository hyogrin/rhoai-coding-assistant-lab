...
The coding assistant stack in this lab — llm-d routing, vLLM model serving, MaaS gateway, and MCP tool servers — deploys on any OpenShift cluster with RHOAI 3.3+. Production teams commonly choose between two managed platforms:
...
Deploy via `LLMInferenceService` as covered in Phase 4. ROSA clusters with RHOAI 3.3 include the llm-d operator and CRD out of the box.
...
| **Benchmark baselines** | Re-run Phase 6 benchmarks per platform; do not extrapolate across GPUs |
...
→ Return to Phase 6 benchmarks to establish per-platform capacity baselines before scaling.
